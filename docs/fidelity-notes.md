...
| 15 | 署名付き URL の Resource とリクエストの一致判定 | canned: scheme を https→http の順で再構成して署名一致を確認（host は `--public-host`（必須）を**ポート込みで verbatim** 使用、path は `EscapedPath()`＝署名時の percent-encoding を保持しデフォルトルートオブジェクト解決後、query は署名パラメータ Expires/Signature/Key-Pair-Id/Policy/Hash-Algorithm を除いたアプリのクエリ）。custom: Resource が `scheme://host/...` を含む場合は **host も照合**（host は canned と同じ `resourceHost(r, publicHost)`＝`--public-host` 設定時はそれ、未設定時はリクエスト Host。scheme は canned 同様 https→http の二択で再構成しワイルドカード照合）。path のみの Resource は path だけ照合（後方互換）。query は無視（path は canned 同様 `EscapedPath()`＝デフォルトルートオブジェクト解決後） | 実 CloudFront の resource 照合の厳密さ（host のポート/サブドメインワイルドカードの扱い、エラーページ等で query を含むか） | 一部実装（2026-06-17: custom の host 照合を追加。`https://*.tenants.example.com/*` 形のワイルドカードサブドメインを host 込みで強制）。canned の host は `--public-host`（環境変数 `LOCALFRONT_PUBLIC_HOST`）を verbatim 使用し**ポートも署名対象に含める**（実 CloudFront は 443/80 のみで配信するため署名 URL にポートが現れないだけで「ポートを剥がす」仕様は存在しない。ローカルは到達アドレスに対して署名する前提）。escaped path 照合は canned/custom で対称。query 照合のみ canned=含む / custom=無視で非対称。実 CloudFront の host 照合厳密さは未対照 |
