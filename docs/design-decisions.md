...
- **名前照合の大文字小文字**: ヘッダ名は HTTP 仕様どおり case-insensitive で照合（`ListSelection.ContainsFold`）、Cookie 名とクエリ文字列名は CloudFront に合わせ case-sensitive で照合（`ListSelection.Contains`）。whitelist/allExcept の両方に適用。
...
- **オリジントランスポートの暗黙 gzip 無効化**: カスタムオリジン (`newTransport`) と S3 (`http.DefaultTransport` のクローン) はいずれも `DisableCompression: true`。Go の `http.Transport` は `Accept-Encoding` 未設定時に自動で `gzip` を付与し透過解凍するため、これを無効化しないと (1) policy が落とした `Accept-Encoding` をオリジンに注入し、(2) `Content-Encoding: gzip` のオブジェクトを勝手に解凍してしまう。CloudFront は policy が選択した時のみ `Accept-Encoding` を送り、オリジン応答は verbatim 転送するため、それに合わせる。
...
- **custom error responses**: オリジンが実際に返したステータス（4xx/5xx）にのみ適用（オリジン接続失敗の合成 502 には適用しない）。`ResponsePagePath` 指定時はそのパスで behavior を再選択してページを取得し、`ResponseCode` で返す（SPA フォールバックの典型）。**オリジンリクエストは再選択した behavior（`pageBeh`）の cache / origin request policy で組み直す**（`BuildOriginRequest(pageBeh, pageReq, ...)`）。失敗したリクエストの behavior の転送設定（ヘッダ/クエリ）をエラーページのオリジンに漏らさないため。未指定時はステータスのみ書き換えてオリジン本文を維持。`ErrorCachingMinTTL` は PoC が無キャッシュのため無視。再取得は 1 段のみ（無限ループ防止）。
...
- **KVS**: インメモリ。`ImportSource`（S3 ARN をストアから取得）と `--kvs-seed <store>=<file>`（Name または logical ID で一致）。`--kvs-seed` が一致したストアは ImportSource を**フェッチせず**シードで置き換える（オフライン／`--s3-endpoint` 無しでも本番 import の代替として使える。起動時・リロード時とも同様）。シードは AWS bulk 形式 `{"data":[{key,value}]}` とフラット `{key:value}` の両方を受理。
...
- **canned policy**: 署名対象 policy が Resource URL を埋め込むため、localfront は `<scheme>://<host><path>[?<query>]` を **https → http の順**で再構成して署名一致を試す（ローカルは平文 HTTP のため scheme が曖昧 — fidelity note 15）。`<host>` は **`--public-host`（必須。環境変数 `LOCALFRONT_PUBLIC_HOST`）をポート込みで verbatim** 使用する（未指定で内部的に空の場合のみ Host ヘッダをそのまま）。実 CloudFront は署名された Resource URL 文字列をリクエストから再構成した URL とそのまま照合するだけで「ポートを剥がす」処理は無い（本番は 443/80 のみで配信するため署名 URL にポートが現れないだけ）。localfront も同様に、開発者が署名した authority（=到達アドレス、ポート含む）をそのまま照合対象にする。query は SDK が署名するベース URL のもの（アプリのクエリ）で、CloudFront の署名パラメータ（`Expires` / `Signature` / `Key-Pair-Id` / `Policy` / `Hash-Algorithm`）を除去し残りを元の順序・エンコードのまま連結する。一致後に `Expires` を検証。policy JSON は SDK のエンコード（`json.Encoder` の HTML エスケープ無効・空白除去・固定フィールド順）をバイト単位で再現する（Resource 文字列も `SetEscapeHTML(false)` で encode し、クエリ中の `&` などが JSON の Unicode エスケープ（コードポイント U+0026 等）に変換されないようにする）。
