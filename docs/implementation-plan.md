...
- **パッケージ追加**: `internal/behavior`（path/policy/CORS/圧縮/viewer ヘッダ）、`internal/sign`（署名付き URL/Cookie）、`internal/accesslog`（CloudFront Standard 形式のアクセスログ出力）。
...
- **アクセスログ**: `--access-log FILE`（環境変数 `LOCALFRONT_ACCESS_LOG`）で CloudFront Standard 形式（33 列タブ区切り + `#Version:` / `#Fields:` ヘッダ）を出力。デフォルトは `-`（stdout）で、空文字を渡すと無効化。本番 S3 配信ログと互換なので既存 ETL がそのまま動く。起動サマリは stderr に移動済 (stdout はログ専用ストリーム)。`x-edge-result-type` は `Miss`（既定）/ `Error`（4xx+5xx）/ `FunctionGeneratedResponse`（viewer-request function 直接応答）の 3 値。Real-time logs (Kinesis 形式) は未実装。
